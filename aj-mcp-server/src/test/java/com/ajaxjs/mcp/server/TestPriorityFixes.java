...
class TestPriorityFixes {
    ...
    void closesSessionsWithoutGetAndRejectsOpeningAfterClose() throws Exception {
        ...
        server.getPendingClientResponses().put(sid + ":1", pending);
        transport.close();
    ...
    void oldDisconnectDoesNotCloseReplacementAndHeartbeatDetectsBrokenWriter() {
        ...
        StringWriter live = new StringWriter();
        PrintWriter current = new PrintWriter(live);
        transport.openEventStream(sid, current, h);
        transport.closeEventStream(sid, old);
        transport.send(sid, "{}");
        assertTrue(live.toString().contains("data: {}"));
        current.close();
        CompletableFuture<JsonNode> pending = new CompletableFuture<>();
        server.getPendingClientResponses().put(sid + ":9", pending);
    ...
    void completionContextRoundTripsAndReachesProvider() {
        CompleteRequest.Params params = new CompleteRequest.Params();
        params.setContext(Collections.singletonMap("language", "python"));
    ...
    void loggingFiltersEachSessionIndependently() {
        Map<String, String> a = session(), b = session();
        StringWriter aw = new StringWriter(), bw = new StringWriter();
        ...
        server.publishLog("info", "test", "info-only");
        server.publishLog("error", "test", "both");
        assertFalse(aw.toString().contains("info-only"));
        assertTrue(bw.toString().contains("info-only"));
        assertTrue(aw.toString().contains("both"));
        assertTrue(bw.toString().contains("both"));
        transport.delete(a.get("Mcp-Session-Id"), a);
        assertFalse(server.getSessionLoggingLevels().containsKey(a.get("Mcp-Session-Id")));
    }
    ...
    void stdioUsesUtf8AndBoundedExecutor() throws Exception {
        ...
        server.getServerConfig().setStrictLifecycle(false);
        server.getServerConfig().setStdioWorkers(1);
        server.getServerConfig().setStdioQueueCapacity(2);
        try {
    ...
    void saturatedStdioStillAcceptsPingAndCancellation() throws Exception {
        ...
        server.getServerConfig().setStrictLifecycle(false);
        server.getServerConfig().setStdioWorkers(1);
        server.getServerConfig().setStdioQueueCapacity(1);
        ...
        try {
            ...
            ServerStdio stdio = new ServerStdio(server);
            try {
                java.lang.reflect.Method dispatch = ServerStdio.class.getDeclaredMethod("dispatchLine", String.class);
                ...
                stdio.getRequestExecutor().shutdown();
                assertTrue(stdio.getRequestExecutor().awaitTermination(2, TimeUnit.SECONDS));
                assertTrue(new String(output.toByteArray(), StandardCharsets.UTF_8).contains("\"isError\":true"));
            } finally {
                com.ajaxjs.mcp.server.cancellation.CancellableTools.release.countDown();
                stdio.close();
            }
        } finally {
            System.setIn(savedIn);
