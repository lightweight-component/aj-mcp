...
This is a Spring Boot 2.7 web application with AJ-MCP Server running over the legacy HTTP/SSE transport. It demonstrates how to keep the MCP implementation framework-neutral while adapting it to Spring MVC controllers.
...
Connect with `HttpMcpTransport`; the server sends an `endpoint` event that points the client to `/message?uuid=<session-id>`:
...
- In a production application, close `ServerSse` from a Spring destruction hook so its heartbeat executor and sessions are released.
- This sample demonstrates the legacy HTTP/SSE transport. For a new HTTP integration, prefer `ServerStreamableHttp` where client compatibility allows it.
