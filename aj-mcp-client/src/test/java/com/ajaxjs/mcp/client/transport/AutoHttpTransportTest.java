...
class AutoHttpTransportTest {
    ...
    @Test
    void modernServerDoesNotProbeLegacy() throws Exception {
        ...
        assertFalse(transport.isLegacySse());
    ...
    @ParameterizedTest
    @ValueSource(ints = {400, 404, 405, 415})
    void fallsBackAndPreservesHeadersAndHandlers(int rejection) throws Exception {
        ...
        start(e -> {
            ...
            if ("/entry".equals(e.getRequestURI().getPath())) {
                reply(e, rejection, null);
                return;
            }
            JsonNode request = body(e);
        ...
        client.onNotification("notifications/message", notice::complete);
        client.initialize();
        assertTrue(transport.isLegacySse());
        assertEquals(selectedVersion, client.getNegotiatedProtocolVersion());
        PingRequest ping = new PingRequest();
        ping.setId(9L);
        ...
        assertNotNull(notice.get(2, TimeUnit.SECONDS));
        assertTrue(headersPresent.get());
        transport.close();
        transport.close();
    }
    ...
    @Test
    void jsonRpcErrorDoesNotTriggerFallback() throws Exception {
        AtomicInteger gets = new AtomicInteger();
        start(e -> {
            if ("GET".equals(e.getRequestMethod())) gets.incrementAndGet();
    ...
    @Test
    void closeInterruptsLegacyEndpointDiscovery() throws Exception {
    ...
    @Test
    void crossOriginEndpointIsRejectedBeforePostingCredentials() throws Exception {
