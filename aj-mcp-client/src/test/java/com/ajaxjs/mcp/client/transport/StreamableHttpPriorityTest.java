...
class StreamableHttpPriorityTest {
    ...
    private void start(HttpHandler handler, boolean get) throws Exception {
        ...
        http.createContext("/mcp", handler);
        http.start();
    ...
    private static void await(CountDownLatch latch) throws IOException {
        try {
            if (!latch.await(4, TimeUnit.SECONDS)) throw new IOException("test latch timed out");
    ...
    void get404AlsoRebuildsSessionAndDropsOldEventCursor() throws Exception {
        ...
        assertEquals("session-2", transport.getSessionId());
    ...
    void expiredSessionIsRebuiltWithoutReplayingTheFailedRequest() throws Exception {
        ...
        client.initialize();
        PingRequest first = new PingRequest();
        first.setId(20L);
        ...
        PingRequest second = new PingRequest();
        second.setId(21L);
    ...
    void negotiatedVersionPrecedesInitializedAndDelete() throws Exception {
        ...
        start(e -> {
            if ("DELETE".equals(e.getRequestMethod())) {
                deleteVersion.set(e.getRequestHeaders().getFirst("MCP-Protocol-Version"));
            ...
            if ("initialize".equals(request.path("method").asText())) initializeReply(e);
                ...
                initializedVersion.set(e.getRequestHeaders().getFirst("MCP-Protocol-Version"));
                reply(e, "2025-06-18".equals(initializedVersion.get()) ? 202 : 400, null);
            }
        }, false);
        McpClient client = McpClient.builder().transport(transport).protocolVersion("2025-03-26").build();
        client.initialize();
        assertEquals("2025-06-18", client.getNegotiatedProtocolVersion());
        assertEquals("2025-06-18", initializedVersion.get());
        client.close();
        client.close();
        assertEquals(1, deletes.get());
        assertEquals("2025-06-18", deleteVersion.get());
    }
    ...
    void get405DoesNotFailConcurrentPost() throws Exception {
        ...
        start(e -> {
            ...
            if ("initialize".equals(method)) initializeReply(e);
                ...
                await(release);
                reply(e, 200, "{\"jsonrpc\":\"2.0\",\"id\":2,\"result\":{}}");
            }
        }, true);
        ...
        try {
            ...
            assertNotNull(transport.getEventStreamFailure());
            release.countDown();
            assertEquals(2, response.get(2, TimeUnit.SECONDS).path("id").asInt());
        } finally {
            release.countDown();
    ...
    void reconnectsClosedGetWithLastEventId() throws Exception {
        ...
        CompletableFuture<String> resumed = new CompletableFuture<>();
        ...
        start(e -> {
            ...
            if ("GET".equals(e.getRequestMethod())) {
                ...
                e.sendResponseHeaders(200, 0);
                if (connection == 1) {
                    e.getResponseBody().write("id: 42\ndata: {\"jsonrpc\":\"2.0\",\"method\":\"notifications/message\",\"params\":{}}\n\n".getBytes(StandardCharsets.UTF_8));
                    e.getResponseBody().flush();
                    e.close();
                } else {
                    resumed.complete(e.getRequestHeaders().getFirst("Last-Event-ID"));
                    e.getResponseBody().write(": ready\n\n".getBytes(StandardCharsets.UTF_8));
                    e.getResponseBody().flush();
                    try {
                        await(release);
                    } finally {
        ...
        try {
            client.initialize();
            transport.getEventStreamReady().get(2, TimeUnit.SECONDS);
            assertEquals("42", resumed.get(3, TimeUnit.SECONDS));
        } finally {
            release.countDown();
        }
    }
